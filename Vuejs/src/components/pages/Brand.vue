<template>
    <div class="brand">
        <ycolumn bgclass="background-blacker" bgimg="" yclass="everyDevice text-white padding-4x-top padding-4x-bottom">
            <div class="col-sm-8">
                <div class="container-inner">
                    <div class="parsys">
                        <div class="richtext">
                            <div class="width-100 component-left">
                                <h1>{{title}} - {{$route.params.name}}</h1>
                                <h2>{{subtitle}}</h2>
                                <p>{{describe}}</p>
                            </div>
        ...
        <ystickynav :items="{'counterbalanceforklifts': '平衡重叉车', 'verynarrowaisletrucks': '特窄通道叉车', 'reachtrucks': '前移式叉车', 'orderpickers': '拣选车', 'stackers': '堆高机', 'pallettrucks': '托盘搬运车', 'towtractors': '牵引车'}"></ystickynav>
...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      // console.log(this.$route.params.name) // router params in script
    })
  }
}
</script>
